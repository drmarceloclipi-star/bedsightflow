...
test.describe('Audit Log Completeness — Field Content Verification', () => {
    ...
    test('UPDATE_BOARD_SETTINGS log has all required fields with correct content', async ({ page }) => {
        ...
        await saveBtn.click();
        await fillConfirmModal(page, 'Motivo E2E completude tv');
        ...
        const boardLog = logs.find(d =>
            str(d.fields, 'action') === 'UPDATE_BOARD_SETTINGS' &&
            str(d.fields, 'reason') === 'Motivo E2E completude tv'
        );
        if (!boardLog) {
        ...
        expect(str(f, 'reason')).toBe('Motivo E2E completude tv');

        // Actor must have all 4 fields (including displayName — fixed in this session)
        expect(actorField(boardLog, 'uid'), 'actor.uid must be present').toBeTruthy();
        expect(actorField(boardLog, 'email'), 'actor.email must be present').toBeTruthy();
        expect(actorField(boardLog, 'displayName'), 'actor.displayName must be present').toBeTruthy();
        expect(actorField(boardLog, 'role')).toBe('admin');
    ...
    test('APPLY_CANONICAL_BEDS log has correlationId field', async ({ page }) => {
        ...
        await fillConfirmModal(page, 'Motivo E2E completude leitos');
        ...
        const applyLog = logs.find(d =>
            str(d.fields, 'action') === 'APPLY_CANONICAL_BEDS' &&
            str(d.fields, 'reason') === 'Motivo E2E completude leitos'
        );
        if (!applyLog) {
        ...
        const corrId = str(applyLog.fields, 'correlationId');
        ...
        expect(str(applyLog.fields, 'reason')).toBe('Motivo E2E completude leitos');
        expect(actorField(applyLog, 'displayName')).toBeTruthy();
    });
    ...
    test('SOFT_RESET_UNIT log has correlationId field', async ({ page }) => {
        ...
        const isVisible = await resetBtn.isVisible({ timeout: 8000 }).catch(() => false);
        if (!isVisible) { test.skip(); return; }

        await resetBtn.click();
        await fillConfirmModal(page, 'Motivo E2E completude soft reset', 'RESETAR');
        ...
        const resetLog = logs.find(d =>
            str(d.fields, 'action') === 'SOFT_RESET_UNIT' &&
            str(d.fields, 'reason') === 'Motivo E2E completude soft reset'
        );
        if (!resetLog) {
